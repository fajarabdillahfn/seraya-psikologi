...
**Acceptance:** incomplete profile blocks booking; client must be 18+ based on date of birth; Indonesian WhatsApp number is validated in E.164/approved local format; client can read/update only their own profile; fields have explicit purpose/privacy mapping; no extra fields added.
...
**Scope:** Beranda, Layanan hover/focus submenu with four program briefs, List Psikolog, About, FAQ, placeholder assets, and catalog/pricing.
...
**Purpose:** Generate valid slots and establish the minimum staff membership foundation required by Admin actions.

**Boundary:** Keep slot generation separate from staff authorization. The staff foundation here is only the membership schema, role enum, and test identity needed by later Admin commands; full staff security hardening is S09.
...
**Acceptance:** Monday–Sunday; public holidays and exceptions closed; windows 09.00–12.00 and 16.00–20.00 WIB; 60-minute sessions; 15-minute buffer-aware starts; 2-hour cutoff; same grid online/offline; overlap/expiry safe. Morning starts are exactly 09.00 and 10.30; evening starts are exactly 16.00, 17.30, and 18.45. `StaffMembership` foundation supports one Superadmin, Admin, and Psychologist.

**Admin schedule capability:** Admin can set public holidays, recurring closed days, and one-off slot withdrawal with an internal reason; the public slot list uses the same source of truth.
...
**Acceptance:** all profile/address and intake fields required; client must be 18+; topics multi-select; problem description minimum 50 and max bounded; consent version stored; no clinical fields; OfferSnapshot/hold/reservation atomic.
...
**Acceptance:** one Superadmin; Admin; Psychologist; Google SSO + StaffMembership; placeholder auth disabled in production; psychologist query is restricted to `booking.state = 'confirmed'` and the authenticated psychologist's assigned bookings; psychologist receives only display name, WhatsApp, mode, slot, topics, non-clinical description, expected outcome, returning-client flag; excludes full address, religion, occupation, education, payment evidence; sharing audit includes actor, timestamp, booking, fields shared; last-admin/superadmin protections.
...
1. **Before S02/S03:** Google SSO login gate and Profile Completion (`Profil Saya` + `Alamat Saya`).
2. **Before S04:** Public navigation, Layanan hover/focus submenu, psychologist card, FAQ, and placeholder assets.
3. **Before S05:** Slot selection calendar, buffer-aware starts, closed-day states, cutoff notice, and Admin holiday/withdrawal calendar.
4. **Before S06:** Booking summary, intake form, multi-topic selector, 50-character counter, consent/safety states.
5. **Before S07:** Preliminary payment handoff and Admin Payment Verification Queue, including proof viewer and amount mismatch notes.
6. **Before S08:** Admin Cancellation/Refund Queue, trigger selector, evidence upload, refund proof, and reschedule adjustment.
7. **Before S09:** Scoped Psychologist View with the customer-data whitelist visibly enforced.

Each checkpoint must include a small wireframe/prototype, mobile review, states, and mapping to PRD acceptance criteria. Do not design deferred features.
...
- [x] Fixed buffer-aware start examples are embedded in S05: 09.00, 10.30, 16.00, 17.30, 18.45 WIB.
...
Until all relevant items are reviewed, this remains a planning artifact only. Review of N11 does not authorize implementation; N12 requires a separate explicit user instruction.
