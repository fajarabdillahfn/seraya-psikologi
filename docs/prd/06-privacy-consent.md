...
- The **Admin shares data with the psychologist only as needed** for an individual booking. Concretely: when a booking reaches `confirmed`, the psychologist (Fuja Rahayu Kinanti for launch) is given the client display name, age range, and counseling topics for that booking, plus the client WhatsApp number to start the Chat or Call session. The psychologist does not see the full address, religion, or any other field that is not required to deliver the session.
